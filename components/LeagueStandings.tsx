...
// ── Shared helpers (mirrored from MyRoster) ──
...
function formatNextStart(start?: ProbableStart | null): { text: string; className: string } {
  ...
  const diffDays = Math.round((gameDay.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
...
function formatMatchup(matchup?: TodayMatchup | null): { text: string; className: string } {
  ...
  const hand = matchup.opposingPitcherHand ? `${matchup.opposingPitcherHand}HP ` : ''
  return { text: `${prefix} ${hand}${matchup.opposingPitcher}`, className: 'text-slate-200' }
}
...
function buildBvpStatRecord(bvp: BvpStats): Record<string, number | string> {
  return {
    bvp_ab: bvp.ab, bvp_h: bvp.h, bvp_hr: bvp.hr, bvp_rbi: bvp.rbi,
    bvp_bb: bvp.bb, bvp_k: bvp.k, bvp_avg: bvp.avg, bvp_ops: bvp.ops,
  }
}
...
const DATE_RANGE_OPTIONS: DateRangeOption[] = [
  { id: 'today', label: 'Today', dateRange: `date=${todayStr()}` },
...
function TeamRosterView({
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true }
  }, [teamKey, leagueKey, activeRange])

  const fetchRecap = useCallback(
    (rosterPlayers: RosterPlayerEntry[], name: string, rangeId: string) => {
      if (rosterPlayers.length === 0) { setRecapSummary(null); return }
  ...
  useEffect(() => {
    if (isFutureDay) { setRecapSummary(null); return }
    if (players.length > 0 && teamName) {
      fetchRecap(players, teamName, selectedRange)
    }
  }, [players, teamName, selectedRange, fetchRecap, isFutureDay])
    ...
    <div className="flex-1 overflow-auto">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-4 px-1">
          <div>
            <h2 className="text-lg font-bold text-white">{teamName ?? 'Team Roster'}</h2>
